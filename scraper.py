...
def get_price_history(conn: sqlite3.Connection, game_id: int) -> list[dict]:
    """Devuelve el historial completo de precios de un juego ordenado por fecha."""
    cur = conn.execute(
    ...
    return [
        {
            ...
            "date":           r["scraped_at"][:10],  # solo YYYY-MM-DD para la gráfica
        }
        for r in cur.fetchall()
    ]


def group_games(rows: list[dict], conn: sqlite3.Connection) -> list[dict]:
    """
    Agrupa filas con el mismo nombre normalizado.
    Cada juego queda con 'prices': lista de {source, url, price, original_price,
    discount_pct, history[]}.
    history[] contiene todos los registros de price_history para esa URL,
    ordenados por fecha — listo para pintar la gráfica.
    """
    ...
    for r in rows:
        ...
        # Historial real de esta URL concreta desde price_history
        history = get_price_history(conn, r["id"])
